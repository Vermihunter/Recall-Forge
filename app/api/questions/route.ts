...
export async function POST(req: Request) {
  ...
  const options =
    format === "multi_select" && Array.isArray(body.options)
      ? body.options.slice(0, 4).map(String)
      : [];
  if (format === "multi_select" && options.length !== 4)
    return NextResponse.json(
      { error: "multi_select questions require exactly four options" },
      { status: 400 },
    );
  const correctOptionIndexes =
    format === "multi_select" && Array.isArray(body.correctOptionIndexes)
      ? Array.from(
          new Set<number>(
            body.correctOptionIndexes
              .map(Number)
              .filter((n: number) => Number.isInteger(n) && n >= 0 && n < 4),
          ),
  ...
  const optionFeedback =
    format === "multi_select"
      ? Array.from({ length: 4 }, (_, i) => {
          const x = Array.isArray(body.optionFeedback)
            ? body.optionFeedback[i]
            : null;
          return {
  ...
  const [question] = await db
    ...
    .values({
      ...
      relatedQuestionIds: Array.isArray(body.relatedQuestionIds)
        ? body.relatedQuestionIds.map(String)
        : [],
      prerequisiteQuestionIds: Array.isArray(body.prerequisiteQuestionIds)
        ? body.prerequisiteQuestionIds.map(String)
        : [],
      keyPoints: Array.isArray(body.keyPoints) ? body.keyPoints : [],
      hints: Array.isArray(body.hints) ? body.hints : [],
      commonMistakes: Array.isArray(body.commonMistakes)
        ? body.commonMistakes
        : [],
