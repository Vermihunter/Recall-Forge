...
function dueScore(q: any) {
  ...
  return (
    (q.repetitions ? 0 : 6) +
    (due ? 4 : 0) +
    Math.max(0, 6 - Number(q.difficulty || 1)) * 0.05
  );
}
...
export async function GET() {
  const rows = await db
    ...
    .orderBy(desc(revisionSessions.updatedAt))
    .limit(50);
...
export async function POST(req: Request) {
  ...
  const sourceSessionSlugs: string[] = Array.from(
    new Set<string>(
      (Array.isArray(body.sourceSessionSlugs) ? body.sourceSessionSlugs : [])
        .map(String)
        .filter(Boolean),
    ),
  );
  const explicitQuestionIds: string[] = Array.from(
    new Set<string>(
  ...
  const targetQuestionCount = Math.max(
    5,
    Math.min(100, Number(body.targetQuestionCount || 30)),
  );
  if (!sourceSessionSlugs.length && !explicitQuestionIds.length)
    return NextResponse.json(
      { error: "Choose at least one roadmap session or recommended question." },
      { status: 400 },
    );
  ...
  if (explicitQuestionIds.length) {
    selected = await db
      .select()
      .from(questions)
      .where(inArray(questions.id, explicitQuestionIds));
    for (const q of selected) origins.set(q.id, "recommended");
  } else {
    const sourceQuestions = await db
      .select()
      .from(questions)
      .where(inArray(questions.sessionSlug, sourceSessionSlugs));
    const sourceSorted = sourceQuestions
      .slice()
      .sort((a, b) => dueScore(b) - dueScore(a));

    const sourceRows = await db
      .select({ slug: roadmapSessions.slug, topicSlug: topics.slug })
    ...
    const neighborhood = topicSlugs.length
      ? await db
          .select()
          .from(questions)
          .where(inArray(questions.topicSlug, topicSlugs))
      : sourceQuestions;
    ...
    const recommendations = rankRecommendations(
      ...
      neighborhood,
      attempts,
      Math.max(6, Math.ceil(targetQuestionCount * 0.3)),
    ).filter((x) =>
      x.reasons.some((reason) =>
        /weak|prerequisite|neighbor|downstream/i.test(reason),
      ),
    );
    const recIds = new Set(recommendations.map((x) => x.question.id));

    const recommendedQuota = recommendations.length
      ? Math.min(Math.ceil(targetQuestionCount * 0.25), recommendations.length)
      : 0;
  ...
  if (!selected.length)
    return NextResponse.json(
      { error: "No questions exist for the selected sources yet." },
      { status: 400 },
    );
  const [created] = await db
    ...
    .values({
      name: String(
        body.name ||
          `Revision · ${sourceSessionSlugs.length || selected.length} sources`,
      ).trim(),
      sourceSessionSlugs,
      targetQuestionCount,
  ...
  await db.insert(revisionSessionItems).values(
    selected.slice(0, targetQuestionCount).map((q, index) => ({
  ...
  return NextResponse.json(
    {
      ...created,
      questionCount: Math.min(selected.length, targetQuestionCount),
    },
    { status: 201 },
  );
}
